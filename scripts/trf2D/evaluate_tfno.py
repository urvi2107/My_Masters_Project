...
def main():
    ...
    CHECKPOINT_PATH = os.path.join(PROJECT_ROOT, "checkpoints", "best_model_tfno.pt")
    if len(sys.argv) > 1:
    ...
    np.save("results/tfno_vrmse.npy", avg_per_step_vrmse_fields)
    np.save("results/tfno_nrmse_spectral.npy", nrmse_spectral)
    wandb.finish()
