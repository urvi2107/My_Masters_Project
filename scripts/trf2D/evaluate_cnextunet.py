...
def main():
    ...
    CHECKPOINT_PATH = os.path.join(PROJECT_ROOT, "checkpoints", "best_model_cnextunet_lr1e-3_epoch280_vrmse0.1934.pt")
    if len(sys.argv) > 1:
    ...
    np.save(os.path.join(results_dir, "cnextunet_vrmse.npy"), avg_per_step_vrmse_fields)
    np.save(os.path.join(results_dir, "cnextunet_nrmse_spectral.npy"), nrmse_spectral)
