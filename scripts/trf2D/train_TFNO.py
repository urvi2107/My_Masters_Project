...
def main():
    parser = argparse.ArgumentParser(description="Train TFNO on TRF2D")
    ...
    # TFNO config matching The Well benchmark
    model = TFNO(
        dim_in=4 * F,
        dim_out=1 * F,
        n_spatial_dims=2,
    ...
    ckpt_prefix = f"best_model_tfno_{lr_tag}_epoch"
    ...
    final_checkpoint_name = f"final_model_tfno_{lr_tag}_epoch{args.epochs}.pt"
    torch.save(model.state_dict(), os.path.join(checkpoint_dir, final_checkpoint_name))
